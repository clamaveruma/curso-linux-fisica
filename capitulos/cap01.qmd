...
Y dos siglas que te perseguirán en toda la documentación, así que
estrenémoslas ya: esta forma de trabajar — órdenes de texto — es una
**CLI** (*command-line interface*, interfaz de línea de comandos); la
de ventanas y ratón de todos los días es una **GUI** (*graphical user
interface*). No son rivales: son dos puertas a la misma casa, y este
curso te entrega la que te faltaba.
...
reaparece pidiendo la siguiente. Ese ciclo es toda la terminal — y
tiene nombre técnico: **REPL** (*read–eval–print loop*: leer, ejecutar,
imprimir, repetir). No es nuevo para ti: el intérprete de Python, con
su `>>>`, es exactamente otro REPL. Cambia el idioma; el ciclo es el
mismo.
...
![Dos teclas que separan a quien sufre la terminal de quien la habita. Ninguna de las dos te pide memorizar nada.](../imagenes/cap01/fig-superpoderes.svg){#fig-superpoderes fig-alt="A la izquierda, el tabulador completa cd Des en cd Descargas; a la derecha, Ctrl+R busca en el historial por un fragmento"}
...
![Las cuatro teclas del visor. Las mismas valen para `less` y para `htop` — las verás durante todo el curso.](../imagenes/cap01/fig-teclas-man.svg){#fig-teclas-man fig-alt="Una página de manual con las teclas para moverse, buscar con la barra, saltar a la siguiente aparición con n y salir con q"}
...
::: {.pista}
Dentro del visor, la tecla <kbd>/</kbd> busca.
:::
...
ficheros dejan de ser intocables: aprenderás a leerlos, copiarlos,
moverlos y organizarlos en lote, y conocerás el primer comando capaz
de destruir — con su red de seguridad correspondiente. Cierra tu
cuaderno con `exit` y
