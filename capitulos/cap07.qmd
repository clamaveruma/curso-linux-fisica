...
[debian.org](https://www.debian.org/distrib/) — unos 800 MB: lo mínimo
para arrancar el instalador, que después descargará el resto de la red.
El navegador la dejará en `~/Descargas`. Y antes de usarla, un hábito
de físico: comprueba que llegó intacta. Junto a la ISO, Debian publica
un fichero `SHA256SUMS` con la **huella digital** de cada imagen — un
número que cambia si cambia un solo bit del fichero:

```terminal
marcos@portatil:~/Descargas$ sha256sum debian-13.7.0-amd64-netinst.iso
a7ef94ac…  debian-13.7.0-amd64-netinst.iso
...
Verificada, un último paso que **no es opcional**: muévela al almacén
de libvirt, `/var/lib/libvirt/images/` — el mismo directorio donde
vivirá el disco de la máquina:

```terminal
marcos@portatil:~/Descargas$ sudo mv debian-13.7.0-amd64-netinst.iso /var/lib/libvirt/images/
marcos@portatil:~/Descargas$ sudo ls -l /var/lib/libvirt/images/
-rw-rw-r-- 1 marcos marcos 792723456 ago 25 12:31 debian-13.7.0-amd64-netinst.iso
```

::: {.por-dentro}
**◉ Por dentro · Por qué la ISO no puede quedarse en tu casa**

La máquina virtual no la ejecutas tú: la ejecuta un proceso que corre
como `libvirt-qemu`, un usuario de sistema de los del capítulo 4. Y en
tu casa no entra nadie que no seas tú — míralo: `ls -ld ~` dice
`drwxr-x---`, sin permiso de paso para «otros». Si dejas la ISO en
`~/Descargas`, el asistente la acepta sin rechistar, pero al arrancar
la máquina QEMU tropieza con un «Permission denied» y la instalación
no llega a empezar. En `/var/lib/libvirt/images/` mandan root y
libvirt, y cualquiera puede leer. Es el capítulo 4 en carne propia: el
proceso que abre un fichero no siempre eres tú.
:::
...
1. **Medio de instalación local (imagen ISO)** → *Explorar*: se abre
   el almacén `default` — es `/var/lib/libvirt/images/` — con tu
   `debian-13.x-amd64-netinst.iso` dentro (si la lista sale vacía,
   pulsa el botón de refrescar). Elígela: comprobará solo que es
   Debian. No la busques en `~/Descargas` con *Buscar local*: la
   instalación fallaría por permisos, como cuenta la caja anterior.
...
5. Esta última pantalla tiene una casilla, *Personalizar la
   configuración antes de instalar*: déjala **sin marcar** — la
   máquina nace con el firmware clásico, el BIOS, y hoy es justo lo que
   quieres (la caja de la sección siguiente cuenta por qué no UEFI).
   **Finalizar.** La máquina arranca sola: verás en su ventana el menú
   del instalador de Debian sobre fondo azul, con «BIOS mode» en el
   título.
...
máquina: es el CD de instalación; vive en ese mismo directorio y puede
borrarse (con `sudo`) cuando ya no la necesites.
:::
...
::: {.por-dentro}
**◉ Por dentro · Por qué la VM arranca «a la antigua»**

Tu Mint arranca por UEFI (capítulo 6); `debian-lab` arranca con el
firmware clásico, el BIOS. No es nostalgia: es una limitación de
versiones. El UEFI de una máquina virtual guarda su memoria — la lista
de arranque que en tu PC vive en un chip de la placa base — en un
segundo fichero, aparte del disco, y el libvirt de Mint 22 no sabe
fotografiar los dos a la vez: si eliges UEFI, la instantánea se niega
con «internal snapshots of a VM with pflash based firmware are not
supported». Entre un arranque moderno y el «deshacer» de cinco
segundos, el curso elige el deshacer: lo que hay *después* del arranque
es idéntico. La limitación está levantada en libvirt 10.10 y
posteriores — el de Mint 23, previsto para diciembre de 2026; LMDE 7 ya
lo lleva —, aunque aun ahí hará falta pedir expresamente que la memoria
del UEFI se guarde en un formato con «versiones» (qcow2, el del disco).
En el capítulo 8 hay un reto para ver el UEFI en una máquina de usar y
tirar; y en tu PC real, el anexo C.
:::
...
::: {.pista}
`/proc/cpuinfo` con `grep -c`, y `lscpu` con `grep -i`.
:::
...
y que el taller responde.

::: {.pista}
Las dos vías instalan exactamente los mismos paquetes; `usermod -aG`
añade sin quitar los grupos que ya tienes; los grupos nuevos no aparecen
...
`SHA256SUMS`, comprueba que la huella coincide, y deja la ISO donde
libvirt pueda leerla.

::: {.pista}
`sha256sum fichero.iso` calcula; `grep netinst SHA256SUMS` te aísla la
línea correcta para compararla a ojo — o deja que
`sha256sum -c SHA256SUMS 2> /dev/null | grep netinst` lo compare por ti.
«Donde libvirt pueda leerla» es su almacén, `/var/lib/libvirt/images/`.
:::
...
imágenes del fichero que no descargaste. Y después, `sudo mv` de la ISO
a `/var/lib/libvirt/images/`.
...
::: {.pista}
El asistente son cinco pantallas; si algo no encaja con el capítulo,
avanza igual — cualquier decisión se cambia después en la configuración
de la máquina.
:::
...
en el UEFI. Si al arrancar la máquina sale «Permission denied» sobre
la ISO, es que la elegiste en `~/Descargas`: muévela al almacén de
libvirt (sección de la imagen) y en la configuración de la máquina
apunta la unidad de CD al nuevo sitio.
...
Explica en tu cuaderno la diferencia.

::: {.pista}
Los discos de libvirt viven en `/var/lib/libvirt/images/`, y ese
directorio es de root.
:::
...
::: {.pista}
`virsh snapshot-list debian-lab` lista;
`virsh snapshot-delete debian-lab NOMBRE` borra.
:::
...
::: {.pista}
`| grep -i memory`, `| grep vcpu`, `| grep -i qcow2`.
:::
...
el capítulo 6, viendo a GRUB instalarse en el disco. Después la romperás y la reinstalarás hasta que aburra,
