...
particiones a mano con el mapa del capítulo 6, mirando a GRUB
instalarse en el primer sector del disco. Después la romperás a
...
- Decidir un plan de particiones *antes* de tocar el instalador — raíz + swap en la VM, y por qué tu PC real añade una tercera pieza, la EFI.
...
al que hablarás por terminal y, a partir del capítulo 12, por SSH.
...
![El plan de particiones para el disco virtual. Dos particiones y un sector: cada pieza con su oficio del capítulo 6.](../imagenes/cap08/fig-particiones-vm.svg){#fig-particiones fig-alt="Disco virtual de 20 GB dividido en raíz ext4 con casi todo el espacio y swap de 2 GB; GRUB va en el primer sector"}

Dos particiones, y cada decisión la tomas con el capítulo 6 en la
cabeza: la **raíz** (`/`, ext4) se lleva casi todo; y **swap** (2 GB)
es el desahogo de la RAM cuando se llena — en una VM de 4 GB, un
seguro barato. ¿Y la **partición EFI** del capítulo 6, el buzón de
GRUB? Aquí no hay: la VM arranca con el firmware clásico, el BIOS
(capítulo 7), y en ese mundo GRUB no va en una partición sino en el
**primer sector del disco** — el MBR, *master boot record* —, que el
BIOS lee a ciegas al encender. Treinta años funcionó así, y en la VM
sigue valiendo; tu PC real, que arranca por UEFI, sí tendrá la EFI, y
el anexo C te lo recuerda. ¿Y `/home` separado? Es una
...
avanzadas (si no tocas nada en treinta segundos, arranca solo con
síntesis de voz: una flecha lo para). Los dos primeros son el mismo
instalador con dos caras — la gráfica es más cómoda con ratón; la de
texto funciona en cualquier sitio, incluida una consola remota sin
ventanas. Elige la que quieras; las pantallas y las decisiones son
idénticas.

![El menú de arranque de la ISO en la VM: «BIOS mode» en el título. Quien sirve este menú es *isolinux*, un cargador pequeño que viaja en la ISO; GRUB — el del capítulo 6 — llegará cuando lo instales en el disco. *Captura del instalador de Debian 13 (GPL).*](../imagenes/cap08/menu-instalador-bios.png){#fig-menu-iso fig-alt="Menú de arranque del instalador de Debian 13 en modo BIOS, con las opciones Graphical install e Install" width="70%"}
...
2. **Red**: la VM recibe dirección sola de la red NAT — es el DHCP del
   capítulo 10, adelantado. Nombre de la máquina: `debian-lab`. Dominio:
   vacío.
...
calma. Elige **Manual**. Verás el disco — «Disco virtual 1 (vda) -
21.5 GB Virtio Block Device» — sin particiones. ¿21,5? El instalador
cuenta en GB decimales, y tus 20 GiB del capítulo 7 son exactamente
eso. El procedimiento, pantalla a pantalla:

1. Selecciona el disco `vda` → **Crear una nueva tabla de particiones
   vacía** → sí. En modo BIOS no pregunta el tipo: crea una `msdos`,
   la clásica (`gpt` es la que espera UEFI, capítulo 6). Aparece una
   línea «ESPACIO LIBRE» con los 21,5 GB.
2. Selecciona el espacio libre → **Crear una partición nueva** →
   tamaño `19.5 GB` (quedan 2 para la swap) → **Primaria** →
   **Principio** → la pantalla de ajustes ya propone *ext4* con punto
   de montaje `/`: compruébalo → **Se ha terminado de definir la
   partición**.
3. Espacio libre restante → partición nueva → tamaño: el que propone
   (todo lo que queda) → **Primaria** — ojo, esta vez el instalador
   señala *Lógica* por defecto — → en ajustes, *Utilizar como:* cambia
   *ext4* por **área de intercambio** → terminar.
4. Compara la tabla con la figura: `#1 primaria 19.5 GB ext4 /` y
   `#2 primaria 2.0 GB swap` — serán `vda1` y `vda2`. **Finalizar el
   particionado y escribir los cambios en el disco** → «¿Desea
   escribir los cambios?» → **Sí**.
...
- **Cargador de arranque**: la VM arranca con BIOS, así que el
  instalador pregunta *en qué disco* instalar GRUB: `/dev/vda` — el
  disco entero, no una partición. Es el primer sector de la figura del
  plan, el sitio que el BIOS mira al encender.
...
Ya no es el isolinux de la ISO: es GRUB, leído del primer sector de
`vda`, sirviendo *tu* sistema.
...
├─vda1 254:1    0 18,2G  0 part /
└─vda2 254:2    0  1,8G  0 part [SWAP]
marcos@debian-lab:~$ df -h /
S.ficheros     Tamaño Usados  Disp Uso% Montado en
/dev/vda1         18G   1,3G   16G   8% /
...
Léelo como el parte de un experimento que ha salido bien: las dos
...
capítulo 12. Pon la instalación al día con el gesto del capítulo 5
...
Acabas de borrar el kernel — y casi todo GRUB: sus módulos y su menú
viven en `/boot/grub`; solo sobrevive su primer trozo, el del sector
cero. El sistema sigue funcionando *hasta que se apaga*: todo lo que
necesitaba ya estaba cargado en RAM. Pero al reiniciar, la cadena del
capítulo 6 se rompe en el segundo eslabón: ese trozo de GRUB arranca,
busca el resto de sí mismo en `/boot/grub`, no lo encuentra, y te deja
en su consola de emergencia — `grub rescue>`, con un «file not found».
Míralo bien. Es la pantalla que aterroriza a quien no sabe qué es GRUB
— y tú sabes exactamente qué falta y por qué.
...
instales Linux en un PC físico. Cambian cuatro cosas — la ISO viaja
en un pendrive, el disco tiene nombre real y quizá un Windows dentro,
el arranque es UEFI y pide una partición más (la EFI del capítulo 6),
y no hay instantánea que te salve — y por eso existe el
[anexo C](../anexos/anexo-c1-pc-real.qmd): la lista de comprobación
...
de la máquina y contraseña de root (vacía, y por qué).

::: {.pista}
La figura del plan tiene todo; el «por qué» de root está en la sección
de arrancar el instalador.
:::

:::: {.solucion}
::: {.callout-tip collapse="true" title="Solución razonada"}
`vda1` ~18 GB · ext4 · `/` — `vda2` 2 GB · swap; GRUB en el primer
sector de `vda`. Máquina `debian-lab`. Root vacío: Debian
...
el cuaderno cuánto ha tardado y en qué pantalla dudaste.

::: {.pista}
Si en el particionado te pierdes, «Deshacer los cambios realizados a las
...
segunda (ejercicio 8.6) es donde se nota que has aprendido. Por eso
...
Después, actualiza el sistema con `apt`.

::: {.pista}
Son exactamente los comandos de los capítulos 5 y 6, ahora sobre una
instalación tuya.
:::

:::: {.solucion}
::: {.callout-tip collapse="true" title="Solución razonada"}
Las dos particiones montadas según el plan; alrededor de 1,3 GB
...
`recien-instalado`. Compruébala con `virsh` desde tu Mint.
...
que `/boot` está de vuelta.

::: {.pista}
La orden está en la sección «Romper, mirar, restaurar». Antes de
teclearla, mira el prompt: debe decir `debian-lab`.
:::

:::: {.solucion}
::: {.callout-tip collapse="true" title="Solución razonada"}
`sudo rm -rf /boot`, `sudo reboot` → el primer trozo de GRUB arranca
(sigue en el sector cero, que no has tocado) pero no encuentra ni el
resto de GRUB ni el kernel: te deja en `grub rescue>` con un «file not
found». Forzar apagado →
...
convierte «no arranca» en «falta `/boot`; el sector cero sigue ahí»: un
...
::: {.pista}
El plan cambia en una línea: la raíz se queda con `14.5 GB` y `/home`
recibe `5 GB`, ext4, punto de montaje `/home` (el instalador lo propone
solo para la segunda ext4). Crea las tres particiones en orden: raíz,
home, swap — todas **primarias**, aunque te sugiera lógica.
:::

:::: {.solucion}
::: {.callout-tip collapse="true" title="Solución razonada"}
Normalmente la mitad del tiempo de la primera vez, y `lsblk` mostrará
`vda2` montado en `/home`. Renueva la instantánea `recien-instalado`
...
Siguiendo el anexo C, graba la ISO en un pendrive que puedas borrar,
...
sin tocar un solo byte de tu disco.

::: {.pista}
En el anexo tienes las dos vías — la aplicación «Discos» de Mint y la
orden `dd` — y la regla de oro: `lsblk` antes y después de pinchar el
pendrive para estar seguro de su nombre.
:::
...
Esc según la marca) y se elige el pendrive. Verás el menú del
instalador — en modo UEFI tiene otro aspecto que en la VM (anexo C),
pero las mismas opciones; *Ctrl+Alt+Supr* o apagar te devuelve a Mint
intacto.
...
::: {.ejercicio}
**Ejercicio 8.8 · La misma instalación, con UEFI (de usar y tirar)**

`debian-lab` arranca con BIOS por la razón del capítulo 7. Para ver
con tus manos qué cambia con UEFI — que es lo que tendrás en un PC real
—, crea una **segunda** máquina, `debian-uefi` (2 GB de RAM bastan),
marcando en el último paso del asistente *Personalizar la
configuración antes de instalar* → *Vista general* → *Firmware:*
**UEFI** → *Aplicar* → *Comenzar la instalación*. Instálala con el
mismo plan y anota las tres diferencias que veas. Después bórrala
(*Eliminar*, con su disco): sin instantáneas no es sitio para vivir.

::: {.pista}
Las diferencias están en el menú de arranque de la ISO, en el
particionado (el instalador querrá una partición más, y de un tipo
concreto) y en el `lsblk` final. Dentro de la máquina, `efibootmgr -v`
enseña dónde apuntó GRUB su llave.
:::

:::: {.solucion}
::: {.callout-tip collapse="true" title="Solución razonada"}
(1) El menú de la ISO ya no es isolinux sino GRUB, con «UEFI» en el
título. (2) Al particionar, la tabla es `gpt` y el instalador espera
una **partición del sistema EFI** (512 MB, FAT32) antes que nada:
créala tú, o el paso «Cargador de arranque» protestará. (3) `lsblk`
muestra `vda1 … /boot/efi` además de la raíz y la swap, y
`efibootmgr -v` lista una entrada `debian` que apunta a
`\EFI\debian\shimx64.efi` dentro de esa partición — la llave en el
buzón del capítulo 6. Y si en virt-manager intentas una instantánea,
verás el aviso de la caja del capítulo 7. Borra la máquina al acabar.

**Por qué así** — Es el ensayo exacto de lo que te encontrarás en el
anexo C, en una máquina donde equivocarse sigue siendo gratis. Las
tres diferencias caben en una línea del cuaderno, y con eso el UEFI
...
<div class="quiz" id="quiz-cap08">
  ...
  <div class="qz"><p class="qq" data-n="3">En <code>debian-lab</code>, que arranca con BIOS, GRUB se instala en…</p>
    <label><input type="radio" name="z3" value="a"><span>el primer sector del disco entero (<code>/dev/vda</code>, el MBR)</span></label>
    <label><input type="radio" name="z3" value="b"><span>la swap</span></label>
    <label><input type="radio" name="z3" value="c"><span>una partición EFI (FAT32, montada en <code>/boot/efi</code>)</span></label>
    <div class="fb good"><b>Correcto.</b> El sitio que el BIOS lee a ciegas al encender. En tu PC real, que arranca por UEFI, el equivalente es la partición EFI del capítulo 6 — el buzón donde GRUB deja su llave.</div>
    <div class="fb bad"><b>No exactamente.</b> En modo BIOS no hay partición EFI: GRUB va al primer sector del disco entero. La partición EFI es el buzón del mundo UEFI — el de tu PC real y del reto 8.8.</div>
  </div>
  <div class="qz"><p class="qq" data-n="4">Marcaste «Servidor SSH» en la selección de programas para…</p>
    ...
    <div class="fb bad"><b>No exactamente.</b> SSH es la puerta remota: permite que tu Mint entre en la VM por la red. Es la semilla del capítulo 12.</div>
  </div>
  ...
  <div class="qz"><p class="qq" data-n="6">La diferencia más peligrosa entre instalar en la VM y en un PC real es…</p>
    ...
    <div class="fb good"><b>Correcto.</b> Mismo instalador, mismas decisiones — pero el «¿desea escribir los cambios?» es irreversible. De ahí el plan escrito y la lista del anexo C.</div>
...
3.  En `debian-lab`, que arranca con BIOS, GRUB se instala en…
    a)  el primer sector del disco entero (`/dev/vda`, el MBR)
    b)  la swap
    c)  una partición EFI (FAT32, montada en `/boot/efi`)
