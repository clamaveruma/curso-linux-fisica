...
::: {.pista}
La respuesta está en `/etc/os-release`, y `grep PRETTY` la aísla.
:::
...
sistema? ¿Cuáles son personas de verdad?

::: {.pista}
Las personas tienen su casa en `/home` — y `grep` sabe buscar eso.
:::
...
colgado. Y verás la otra mitad del capó: qué hace de verdad el gestor
de software de Mint por debajo — `apt`, repositorios, dependencias — y
por qué a veces `python` no es el Python que crees. Guarda
`sesion04.log`.
:::
