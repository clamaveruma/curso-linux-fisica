...
con más runs), aquí es una orden.
...
como quedó el último día: duplicados, nombres con espacios, un run
...
Después mira dentro de `registro.log` y de las notas: ¿qué run está
...
son el mismo run guardado dos veces.
...
Lleva los cinco runs buenos a `datos/` con nombres uniformes
`run01.csv` … `run05.csv` (recuerda las comillas para los nombres con
espacios), y el run malo a `descartes/` con un nombre que avise:
...
Hoy has renombrado cinco runs a mano y te ha sabido a poco elegante —
bien: esa molestia es la puerta del capítulo 3. Los comandos dejan de
trabajar solos y empiezan a encadenarse: la salida de uno como entrada
del siguiente, `grep`, `sort` y las tuberías. El análisis completo de
un log de adquisición sin abrir Python — y saber cuándo sí conviene
abrirlo. Guarda `sesion02.log`: ya tienes dos medidas en tu cuaderno.
:::
