...
dirección *WAN* y comprueba que coincide con la pública.

::: {.pista}
Si `curl` no está instalado, ya sabes: `sudo apt install curl`.
:::
...
anota los que no reconozcas.

::: {.pista}
Si el router no la muestra o está incompleta, `ip neigh` en tu Mint es
el plan B, y `nmap -sn` sobre tu `/24`, el plan C.
:::
...
::: {.pista}
Mira el `default via` de la VM y búscalo en el `ip a` de tu Mint.
:::
...
fija», en la sección LAN/DHCP; necesita tu MAC (`ip a`, línea
`link/ether`). Si tu router no lo ofrece, deja constancia en el cuaderno
y sigue: no todos lo exponen.
:::
...
::: {.pista}
`ip a` te dirá qué interfaz está activa en cada caso (`wlp…` o `enp…`);
desactiva la Wi-Fi mientras mides por cable para no mezclar.
:::
