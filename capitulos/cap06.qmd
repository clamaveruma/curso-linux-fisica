...
- Qué es la partición EFI que te encontrarás al instalar Debian en un PC real.
:::
...
Todo lo que `lsblk` te cuenta en texto lo dibujan dos aplicaciones
gráficas que conviene conocer por su nombre, porque las verás en
cualquier foro. **Discos** (menú → Accesorios; `gnome-disks`) viene
con Mint: enseña cada disco con sus particiones como una barra de
colores, monta y desmonta con un botón, formatea un pendrive y edita
las opciones de montaje (anexo E). **GParted** (`sudo apt install
gparted`, y viene de serie en el pendrive *live* de Mint) es *el*
editor de particiones: crea, borra, redimensiona y mueve particiones
— es la herramienta con la que se le hace sitio a Linux junto a
Windows, y su hermano de texto es lo que usarás en el instalador del
capítulo 8. Hoy, por la regla del curso, las abres solo para *mirar*:
GParted sobre tu disco real es tan destructivo como potente. Para
tocar, la máquina virtual del bloque C o un pendrive que puedas
borrar.
:::
...
basta con saber *leerlo* — montar a mano discos de otros formatos,
hacerlo permanente y enganchar carpetas de otras máquinas es el
[anexo E](../anexos/anexo-b2-montajes.qmd), para cuando hayas visto
SSH:
...
el bloque C vas a *construir* esta cadena — particionar el disco, ver
a GRUB instalarse — y cuando algo no arranque (a todos nos pasa),
...
Empieza el bloque C, y por fin está permitido romper cosas. En el
capítulo 7 creas tu primera
