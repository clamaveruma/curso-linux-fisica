...
conexión* del icono de red.

::: {.pista}
La MAC es la línea `link/ether`; la IP y la máscara van juntas en
`inet`; la puerta de enlace es el `default via` de `ip route`.
:::
...
está ese servidor de Google.

::: {.pista}
La línea final `rtt min/avg/max` te da la media sin calcular nada.
:::
...
::: {.pista}
Bits para máquinas = 32 − máscara; caben 2 elevado a eso, menos las dos
