...
te dé los comandos (anexo A): lo que este capítulo te da es
...
tmux — y comprueba de nuevo.

::: {.pista}
`ssh lab "ps aux | grep sleep"` es la comprobación sin entrar; la
primera vez desaparecerá, la segunda seguirá ahí.
:::
...
y luego engánchate para verlo terminar.
...
::: {.pista}
El túnel solo existe mientras esa conexión SSH esté abierta.
:::
...
::: {.pista}
La URL con el `token` la imprime Jupyter al arrancar; si la pierdes,
`jupyter notebook list` la repite.
:::
