...
::: {.pista}
`host nombre` a secas; si `host` no existe,
...
copia de seguridad del fichero y vuelta atrás escrita.

::: {.pista}
`sudo cp /etc/hosts /etc/hosts.bak`, `sudo nano /etc/hosts`, y una línea
como `192.168.122.15  debian-lab` al final. Vuelta atrás:
...
::: {.pista}
Privadas son `10.x`, `192.168.x` y `172.16–31.x`; repasa la figura del
NAT del capítulo 10.
:::
...
ya tiene nombre). ¿Coinciden las dos vistas?

::: {.pista}
`-t` TCP, `-l` solo los que escuchan, `-n` números en vez de nombres.
:::
...
debian.org` sigue resolviendo.

::: {.pista}
El DNS suele estar en la pestaña LAN/DHCP o en WAN/Internet del router;
el valor anterior será a menudo «automático» o la IP de la compañía —
anótalo tal cual.
:::
...
apágalo (`sudo ufw disable`) o restaura la instantánea.

::: {.pista}
El orden importa — `allow OpenSSH` antes de `enable`, o te quedas fuera;
